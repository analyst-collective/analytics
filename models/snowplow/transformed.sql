...
CREATE OR REPLACE VIEW snowplow_analysis.com_snowplowanalytics_snowplow_desktop_context_1_transformed as (
  SELECT
  ...
  FROM
    snowplow_analysis.com_snowplowanalytics_snowplow_desktop_context_1_filtered
);



CREATE OR REPLACE VIEW snowplow_analysis.maxmind_ip_isp_transformed as (
  SELECT
    "ip_stop"   as "ip_stop",
  ...
  FROM
    snowplow_analysis.maxmind_ip_isp_filtered
);



CREATE OR REPLACE VIEW snowplow_analysis.com_snowplowanalytics_snowplow_geolocation_context_1_transformed as (
  SELECT
    "speed"                        as "speed",
    "bearing"                      as "bearing",
    ...
    "root_tstamp"                  as "root_tstamp"
  FROM
    snowplow_analysis.com_snowplowanalytics_snowplow_geolocation_context_1_filtered
);



CREATE OR REPLACE VIEW snowplow_analysis.maxmind_isp_names_recaptured_by_yev_transformed as (
  SELECT
    "isp_name"  as "isp_name"
  FROM
    snowplow_analysis.maxmind_isp_names_recaptured_by_yev_filtered
);



CREATE OR REPLACE VIEW snowplow_analysis.com_snowplowanalytics_snowplow_link_click_1_transformed as (
  SELECT
    "root_id"          as "root_id",
    ...
    "root_tstamp"      as "root_tstamp"
  FROM
    snowplow_analysis.com_snowplowanalytics_snowplow_link_click_1_filtered
);



CREATE OR REPLACE VIEW snowplow_analysis.visitors_with_company_name_transformed as (
  SELECT
  ...
  FROM
    snowplow_analysis.visitors_with_company_name_filtered
);



CREATE OR REPLACE VIEW snowplow_analysis.events_hackathon_transformed as (
  SELECT
  ...
  FROM
    snowplow_analysis.events_hackathon_filtered
);



CREATE OR REPLACE VIEW snowplow_analysis.com_snowplowanalytics_snowplow_change_form_1_transformed as (
  SELECT
    "root_id"          as "root_id",
    "value"            as "value",
    "element_classes"  as "element_classes",
    ...
    "root_tstamp"      as "root_tstamp"
  FROM
    snowplow_analysis.com_snowplowanalytics_snowplow_change_form_1_filtered
);



CREATE OR REPLACE VIEW snowplow_analysis.maxmind_investors_lower_names_transformed as (
  SELECT
    "lower_company_name"  as "lower_company_name"
  FROM
    snowplow_analysis.maxmind_investors_lower_names_filtered
);



CREATE OR REPLACE VIEW snowplow_analysis.com_snowplowanalytics_snowplow_submit_form_1_transformed as (
  SELECT
    "root_id"         as "root_id",
    ...
    "root_tstamp"     as "root_tstamp"
  FROM
    snowplow_analysis.com_snowplowanalytics_snowplow_submit_form_1_filtered
);



CREATE OR REPLACE VIEW snowplow_analysis.maxmind_ip_org_transformed as (
  SELECT
    "ip_stop"   as "ip_stop",
    "ip_start"  as "ip_start",
    "key"       as "key",
  ...
  FROM
    snowplow_analysis.maxmind_ip_org_filtered
);



CREATE OR REPLACE VIEW snowplow_analysis.com_snowplowanalytics_snowplow_add_to_cart_1_transformed as (
  SELECT
  ...
  FROM
    snowplow_analysis.com_snowplowanalytics_snowplow_add_to_cart_1_filtered
);



CREATE OR REPLACE VIEW snowplow_analysis.maxmind_ip_org_distinct_lower_names_transformed as (
  SELECT
    "org_name"  as "org_name"
  FROM
    snowplow_analysis.maxmind_ip_org_distinct_lower_names_filtered
);



CREATE OR REPLACE VIEW snowplow_analysis.replay_events_transformed as (
  SELECT
    "dvce_ismobile"             as "dvce_ismobile",
    "br_cookies"                as "br_cookies",
    "br_features_silverlight"   as "br_features_silverlight",
    ...
    "tr_total"                  as "tr_total"
  FROM
    snowplow_analysis.replay_events_filtered
);



CREATE OR REPLACE VIEW snowplow_analysis.maxmind_ip_isp_distinct_lower_names_transformed as (
  SELECT
    "isp_name"  as "isp_name"
  FROM
    snowplow_analysis.maxmind_ip_isp_distinct_lower_names_filtered
);



CREATE OR REPLACE VIEW snowplow_analysis.events_of_visitors_with_legit_company_names_transformed as (
  SELECT
    "user_ipaddress_int"        as "user_ipaddress_int",
    "event_id"                  as "event_id",
    "page_referrer"             as "page_referrer",
  ...
  FROM
    snowplow_analysis.events_of_visitors_with_legit_company_names_filtered
);



CREATE OR REPLACE VIEW snowplow_analysis.com_snowplowanalytics_snowplow_ua_parser_context_1_transformed as (
  SELECT
    "root_id"            as "root_id",
    ...
    "root_tstamp"        as "root_tstamp"
  FROM
    snowplow_analysis.com_snowplowanalytics_snowplow_ua_parser_context_1_filtered
);



CREATE OR REPLACE VIEW snowplow_analysis.transformed_snowplow_events_transformed as (
  SELECT
    "dvce_ismobile"             as "dvce_ismobile",
    "user_ipaddress_int"        as "user_ipaddress_int",
    "domain_sessionidx"         as "domain_sessionidx",
  ...
  FROM
    snowplow_analysis.transformed_snowplow_events_filtered
);



CREATE OR REPLACE VIEW snowplow_analysis.events_transformed as (
  SELECT
    "dvce_ismobile"             as "dvce_ismobile",
    "br_cookies"                as "br_cookies",
    "br_features_silverlight"   as "br_features_silverlight",
...
COMMENT ON TABLE snowplow_analysis.events_filtered IS 'timeseries,funnel,cohort';



CREATE OR REPLACE VIEW snowplow_analysis.maxmind_law_firms_lower_names_transformed as (
  SELECT
    "lower_company_name"  as "lower_company_name"
  FROM
    snowplow_analysis.maxmind_law_firms_lower_names_filtered
);